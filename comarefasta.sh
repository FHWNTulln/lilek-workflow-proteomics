...
grep -F ">" one.fasta | cut -c 5- | sort | tee one.sorted.txt
grep -F ">" two.fasta | cut -c 5- | sort | tee two.sorted.txt
...
grep -F ">sp" three.fasta | cut -c 5- | sort | tee three.sp.txt
grep -F ">sp" five.fasta | cut -c 5- | sort | tee five.sp.txt
...
diff -u three.sp.txt five.sp.txt | tee diff.three.five.txt
...
grep "^-\|^+" diff.three.five.txt | cut -c 2- | sed -e '1,2d'| sort | uniq -w6 | tee -a diff.three.five.uniqueproteins.txt
#grep the unreviewed proteins
echo "############### tr proteins" | tee -a diff.three.five.txt
grep ">tr" three.fasta | cut -c 5- | sort | tee -a diff.three.five.uniqueproteins.txt
