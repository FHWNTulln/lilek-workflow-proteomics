...
# read in files - for each file one variable is created
for (i in 1:length(file.list)) assign(file.list[i], read.delim(paste(path,file.list[i],sep="")))


sample_names_raw <- c("A1","A2","A3")
unique <- 2  #how many petides should be necassary to identify a protein

sample_names <- paste("Razor...unique.peptides.",sample_names_raw, sep="")
...
for (x in file.list){
  ...
  for (i in 1:length(sample_names)){
    ...
    print(name)
     cat("\n")
    print(sum(tmp, na.rm = TRUE))
     cat("\n")
  }
  
  assign(paste("results",x,sep="_"),results) #create individual variable names
...
for (i in seq_along(file.list)) {
    my_data[[i]] <- read.delim(paste(path,file.list[i],sep=""))
}

file.path <- paste(path,file.list,sep="") 
tmp <- lapply(file.path, read.delim)
names(tmp) <- file.list
...
## Comparison of different MaxQuant versions

### Razor

Here only the results are plotted for documentation without discussion. For details on the comparison of different MaxQuant versions using LFQ or Razor look at the section `Summary Comparison of different versions`.
...
### Summary Comparison of different Versions

In conclusion from `210709_MaxQuant_V003.html` the following two things were stated: 

* the older version shows significant more identifications than the newer version (median of the difference new-old = -7)
* because of this always the latest version of MaxQuant should be used

Re-analyzing the LFQ values shows the same results than before (median of the difference new-old = -7). Also using Razor the same conclusio can be drawn (median of the difference new-old = -9) that there's a significant difference in the number of identifications (p=`r round(t$p.value,4)`).

## Comparison rerun

### Razor

Here only the results are plotted for documentation without discussion. For details on the comparison of re-running using LFQ or Razor look at the section `Summary Comparison LFQ-Razor`.
...
Here only the results are plotted for documentation without discussion. For details on the comparison of re-running using LFQ or Razor look at the section `Comparison LFQ-Razor`.
...
### Summary Comparison LFQ-Razor

* for comparison data from experiment 002 and 003 were used and boxplots for the number of protein identification were created
* also the rsd (relative standard deviation) was caclulated an visualized using a boxplot

From the results the following conclusion can be drawn:

* LFQ seems to yield lower no. of protein identificiations compared to Razor independet if MBR is used (002) or not (003)
* The differenc of no. protein identifications between LFQ and Razor in one extract seems to fluctuate (sometimes bigger (A2) or lower (A3)) depending on the extract
```{r}
...
rsd_003 <- apply(results_2gether_lfq[c(15:24),-4],1,CV)
```
* rsd is lower when using lfq compared to Razor
  + median rsd LFQ 002 `r round(median(rsd_002),4)`
  + median rsd Razor 002 `r round(median(rsd_002_r),4)`
  + median rsd LFQ 003 `r round(median(rsd_003),4)`
  + median rsd Razor 003 `r round(median(rsd_003_r),4)`
...
## Comparison different order

As can be seen different orders of the raw-files didn't seem to have an influence on the no. of protein identifications 

* independent if the option fractions was used (experiment 002) or not (experiment 001) and
* independet if Razor or LFQ was used for evaluation.

In both cases the option `match between runs` was used. 
...
* using **different versions** the same conclusion can be drawn using LFQ or Razor -> older version shows more protein identifications
* **re-running** of the same experiment
  + LFQ yields lower protein identificiations than Razor - the magnitude of the difference doesn't correlate with the no. of protein identifications but seems to depend on the extraction/sample
  + rsd for LFQ is lower than for Razor
* **different order** doesn't affect the protein identifications independet if LFQ or Razor is used